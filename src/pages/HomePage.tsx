...
import { ProfileImage,UserNameProfile,ButtonsOptions,LoginProfile,DivOptions,DivMain,BioProfile,DivFollows,Container,ProfileInformations} from "./HomePage.styles"
...
export const HomePage : React.FC = () => {
  ...
  const loadUser = async(userName:string)=>{
    ...
    const {avatar_url, login, bio, public_repos, name, location, followers, following } = data;
      ...
      <DivMain onLoad={()=>onSelectOption(0)}>
        <div>
          <ProfileInformations>
            ...
            <UserNameProfile>{user?.name}</UserNameProfile>
        ...
        <DivFollows>
          <DivOptions style={user?.name?{display:"block",padding:"15px 10px"}:{display:"none"}}>
            <ButtonsOptions onClick={()=>onSelectOption(1)} style={option===1?{backgroundColor:"#E7E7E7"}:{}}>Repositórios <span>{user?.public_repos}</span></ButtonsOptions>
            <ButtonsOptions onClick={()=>onSelectOption(2)} style={option===2?{backgroundColor:"#E7E7E7"}:{}}>Seguidores <span>{user?.followers}</span></ButtonsOptions>
            <ButtonsOptions onClick={()=>onSelectOption(3)} style={option===3?{backgroundColor:"#E7E7E7"}:{}}>Seguindo <span>{user?.following}</span></ButtonsOptions>
            <ButtonsOptions onClick={()=>onSelectOption(4)} style={option===4?{backgroundColor:"#E7E7E7"}:{}}>Estrelas <span>{stars.length}</span></ButtonsOptions>
          </DivOptions>
          <DivOptions style={option!==0?{display:"block"}:{display:"none"}}>
            ...
            <ul style={option===4?{display:"block"}:{display:"none"}}>
              {
